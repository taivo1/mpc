...
void * set_xbee( void * temp_pt )
{
	/**************************************************
	 * Set the xbee configuration.
	 **************************************************/
    ...
    fprintf(stderr,"+++\n");
    sleep(2);
    write(sd,atid,strlen(atid));
    fprintf(stderr,"%s",atid);
    usleep(50000);
    write(sd,atmy,strlen(atmy));
    fprintf(stderr,"%s",atmy);
    usleep(50000);;
    write(sd,atdl,strlen(atdl));
    fprintf(stderr,"%s",atdl);
    usleep(50000);;
    write(sd,atbd,strlen(atbd));
    fprintf(stderr,"%s",atbd);
    usleep(50000);
    write(sd,"atcn\r\n",6);
    end=1;
    return 0;
...
//fprintf(stderr,"Press Ctrl+c to close the program\n");
    ...
    for(i=2;i<6;i++)
    {
        if (strlen(argv[i])>40)
        {
            fprintf(stderr,"Usage: %s [port] [atid] [atmy] [atdl] [atbd]\nOr: %s [port] [atid] [atmy] [atdl] [atbd] [speed]\n", argv[0], argv[0]);
            fprintf(stderr,"An option is oversized. Exiting\n");
            fprintf(stderr,"Author: Octavio Benedi Sanchez\n");
            exit(0);
        }
    }
    ...
    // initializing thread that scans keyboard and writes data to serial line.


     // Two modes, getter and setter.
     if( pthread_create( &thread2, NULL, set_xbee, NULL ) != 0 )
     {
       printf("Cannot create thread2 , exiting \n ");
       exit(-1); // exit with errors
     }
