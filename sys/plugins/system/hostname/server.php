...
{
    $hostname=jsondecode($_POST['form_fields']);    
    $fields_check_types = Array (
          'hostname_value'  => Array ('ms_alnum','ms_mandatory')
        );
    if(are_form_fields_valid ($hostname, $fields_check_types))
    {
        $fp=fopen($base_plugin.'data/hostname','w');
        fwrite($fp,$hostname['hostname_value']);
        fclose($fp);
        exec('sudo cp '.$base_plugin.'data/hostname /etc/hostname');
        exec('sudo hostname '.$hostname['hostname_value']);
