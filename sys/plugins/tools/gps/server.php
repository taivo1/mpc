...
include_once $API_core.'parser_NMEA.php';
include_once $base_plugin.'php/gps_position.php';

if ($_POST['function']=="get_center")
{
    $center=get_pos();
    response_additem("x_pos", $center['lat']);
    response_additem("y_pos", $center['lng']);
    response_return();
}
elseif ($_POST['function']=="show_nmea")
{
    $nmea_data=get_nmea();
    response_additem("nmea_data", $nmea_data);
    response_return();
}
elseif ($_POST['function']=="save_google_key")
{
    if(!empty($_POST['google_key']))
    {
        $fp=fopen($base_plugin.'data/google_key.value','w');
        fwrite($fp,$_POST['google_key']);
        fclose($fp);
        unset($fp);
    }
    else
    ...
    response_additem("return", '<fieldset><h2>Data saved.</h2></fieldset>');
    response_return();
}
