...
{
    $traceroute_data=jsondecode($_POST['form_fields']);
    $out='';
    $fields_check_types = Array (
          'ip_address'  => Array ('ms_host','ms_mandatory')
        );
    if(are_form_fields_valid ($traceroute_data, $fields_check_types))
    {
        $fp=fopen($base_plugin.'data/url.php','w');
        fwrite($fp,"<?php\n \$url='".$traceroute_data['ip_address']."';\n");
        fwrite($fp,'$interface=\''.$traceroute_data['interface']."';\n?>");
        fclose($fp);
        $out.='<iframe class="iframe" src="'.$url_plugin.'php/traceroute.php"></iframe>';
    }
    response_additem("return", '<pre>'.print_r($traceroute_data,true).'</pre>');
